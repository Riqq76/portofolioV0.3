...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function typeEffect() {
    ...
    if (isDeleting) {
      rotatingText.textContent = currentText.substring(0, index--);
    } else {
      rotatingText.textContent = currentText.substring(0, index++);
    }
    ...
    if (!isDeleting && index === currentText.length + 1) {
      ...
      isDeleting = false;
      count = (count + 1) % texts.length; // pindah ke teks berikutnya
      setTimeout(typeEffect, 200);
    } else {
      setTimeout(typeEffect, isDeleting ? 50 : 100); // speed ketik/hapus
    }
  }
